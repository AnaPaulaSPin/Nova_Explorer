...
A pasta **Nova-explorer** contém o modelo inicial da aplicação.

Essa versão foi desenvolvida utilizando:
...
# 🚀 Como executar a aplicação Angular

## 1. Baixar o projeto

Após baixar ou clonar o projeto, localize a pasta:

```text
New-explorer
```

> Os comandos do Angular devem ser executados dentro dessa pasta.
...
Depois de baixar o projeto, é necessário instalar as dependências.
...
> A pasta `node_modules` não precisa ser enviada junto com o projeto, pois pode ser recriada utilizando `npm install`.
...
## Comando `ng` não reconhecido

Caso o terminal informe que o comando `ng` não foi reconhecido, tente executar:
